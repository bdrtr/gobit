...
// already records together with the caller; the other two come from a flow with
// no person behind it. So a reader who wants a name knows from the reason
...
type Movement struct {
	...
	// [MovementSale] and empty on every other reason, which the schema states as
	// an equivalence rather than as an option.
