...
const anonymizeCartAddresses = `-- name: AnonymizeCartAddresses :execrows
...
func (q *Queries) AnonymizeCartAddresses(ctx context.Context, cartIds []string) (int64, error) {
	result, err := q.db.Exec(ctx, anonymizeCartAddresses, cartIds)
	if err != nil {
		return 0, err
	}
...
const anonymizeCartContacts = `-- name: AnonymizeCartContacts :execrows
UPDATE carts
SET email      = NULL,
    updated_at = now()
WHERE id = ANY ($1::text[])
...
func (q *Queries) AnonymizeCartContacts(ctx context.Context, cartIds []string) (int64, error) {
	result, err := q.db.Exec(ctx, anonymizeCartContacts, cartIds)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
const listCartsForErasure = `-- name: ListCartsForErasure :many
...
type ListCartsForErasureParams struct {
	CustomerID *string
	Email      *string
}
...
func (q *Queries) ListCartsForErasure(ctx context.Context, arg ListCartsForErasureParams) ([]string, error) {
	rows, err := q.db.Query(ctx, listCartsForErasure, arg.CustomerID, arg.Email)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
