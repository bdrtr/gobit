...
const listCartAddressesForDisclosure = `-- name: ListCartAddressesForDisclosure :many
SELECT id,
       cart_id,
       source_address_id,
       first_name,
...
type ListCartAddressesForDisclosureRow struct {
	ID              string
	CartID          string
...
func (q *Queries) ListCartAddressesForDisclosure(ctx context.Context, cartIds []string) ([]ListCartAddressesForDisclosureRow, error) {
	rows, err := q.db.Query(ctx, listCartAddressesForDisclosure, cartIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListCartAddressesForDisclosureRow{}
	for rows.Next() {
		var i ListCartAddressesForDisclosureRow
		if err := rows.Scan(
			&i.ID,
			&i.CartID,
			&i.SourceAddressID,
...
const listCartShippingNotesForDisclosure = `-- name: ListCartShippingNotesForDisclosure :many
SELECT id,
       cart_id,
       data
...
type ListCartShippingNotesForDisclosureRow struct {
	ID     string
	CartID string
	Data   []byte
}
...
func (q *Queries) ListCartShippingNotesForDisclosure(ctx context.Context, cartIds []string) ([]ListCartShippingNotesForDisclosureRow, error) {
	rows, err := q.db.Query(ctx, listCartShippingNotesForDisclosure, cartIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListCartShippingNotesForDisclosureRow{}
	for rows.Next() {
		var i ListCartShippingNotesForDisclosureRow
		if err := rows.Scan(&i.ID, &i.CartID, &i.Data); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
const listCartsForDisclosure = `-- name: ListCartsForDisclosure :many
...
type ListCartsForDisclosureParams struct {
	CustomerID *string
	Email      *string
	MaxCarts   int64
}

type ListCartsForDisclosureRow struct {
	ID         string
	CustomerID *string
	Email      *string
	Metadata   []byte
...
func (q *Queries) ListCartsForDisclosure(ctx context.Context, arg ListCartsForDisclosureParams) ([]ListCartsForDisclosureRow, error) {
	rows, err := q.db.Query(ctx, listCartsForDisclosure, arg.CustomerID, arg.Email, arg.MaxCarts)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListCartsForDisclosureRow{}
	for rows.Next() {
		var i ListCartsForDisclosureRow
		if err := rows.Scan(
			&i.ID,
			&i.CustomerID,
			&i.Email,
