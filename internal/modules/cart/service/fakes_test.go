...
// linkPair kurulmuş bir bağın iki ucudur.
...
// fakeLinker service.Linker'ın bellek içi karşılığıdır.
type fakeLinker struct {
	mu    sync.Mutex
	pairs []linkPair
	// failCreate ayarlanırsa Create bu hatayı döner; sepetin geri alınma yolunu
	// sınamak için kullanılır.
	failCreate error
	// failCreateFor yalnızca bu ada sahip bağda hata üretir; boşsa tüm adlar.
	failCreateFor string
}

// Sahte bağ servisinin servisin beklediği yüzeyi karşıladığı derleme zamanında
// doğrulanır.
var _ service.Linker = (*fakeLinker)(nil)

// newFakeLinker boş bir sahte bağ servisi üretir.
func newFakeLinker() *fakeLinker {
	return &fakeLinker{}
}

// Create bağı kaydeder.
func (l *fakeLinker) Create(_ context.Context, name, fromID, toID string) error {
	l.mu.Lock()
	defer l.mu.Unlock()

	if l.failCreate != nil && (l.failCreateFor == "" || l.failCreateFor == name) {
		return l.failCreate
	}
	for _, pair := range l.pairs {
		if pair == (linkPair{name: name, fromID: fromID, toID: toID}) {
			return nil
		}
	}
	l.pairs = append(l.pairs, linkPair{name: name, fromID: fromID, toID: toID})
	return nil
}

// Delete bağı kaldırır.
func (l *fakeLinker) Delete(_ context.Context, name, fromID, toID string) error {
	l.mu.Lock()
	defer l.mu.Unlock()

	l.pairs = slices.DeleteFunc(l.pairs, func(pair linkPair) bool {
		return pair.name == name && pair.fromID == fromID && pair.toID == toID
	})
	return nil
}

// List fromID'ye bağlı toID'leri döner.
func (l *fakeLinker) List(_ context.Context, name, fromID string) ([]string, error) {
	l.mu.Lock()
	defer l.mu.Unlock()

	out := []string{}
	for _, pair := range l.pairs {
		if pair.name == name && pair.fromID == fromID {
			out = append(out, pair.toID)
		}
	}
	slices.Sort(out)
	return out, nil
...
// targets verilen link adındaki tüm bağların hedeflerini döner.
func (l *fakeLinker) targets(name string) []string {
	l.mu.Lock()
	defer l.mu.Unlock()
