...
// idOnlyProvider is a TaxProvider that does nothing but carry an identifier.
//
// The local provider could not be used: every one of them carries the same id
// ([LocalProviderID]), and this test also needs writers with ids of their own.
type idOnlyProvider struct{ id string }

// ID returns the provider's identifier.
func (c *idOnlyProvider) ID() string { return c.id }

// Calculate is never called; the registration contract does not reach it.
func (c *idOnlyProvider) Calculate(context.Context, ProviderInput) (ProviderResult, error) {
...
//
// `ProviderRegistry` promises it is safe for concurrent use. Measured on
// 2026-09-09, eleven packages make a promise like that and not one of the five
// provider registries had a test running the promising type from two
// goroutines. The race detector only looks at code that actually ran
// concurrently, so the `-race` lane had never seen these locks — proved on the
// sibling module, where deleting the lock from Register left the whole package
// green.
//
// # What it asserts
//
...
// afterwards is the winner itself.
func TestTheRegistryKeepsItsPromiseUnderConcurrentUse(t *testing.T) {
	...
	winners := make(chan *idOnlyProvider, registryWriters)
	...
	for i := range registryWriters {
		...
		go func() {
			defer done.Done()
			provider := &idOnlyProvider{id: "contested"}
			start.Wait()
		...
		go func() {
			defer done.Done()
			start.Wait()
			assert.NoError(t, registry.Register(&idOnlyProvider{id: fmt.Sprintf("separate-%d", i)}))
		}()
	}
