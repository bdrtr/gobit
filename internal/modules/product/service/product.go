// The product record itself: its inputs, its listing criteria and the six
// operations that create, read, update and delete it.
//
// It is its own file for the reason every other module's entity file is
// (order/service/order.go, cart/service/cart.go): service.go is the package's
// spine — the dependency contract, the constructor and the one result type
// every listing here returns — and a subject that grows helpers of its own
// belongs beside its siblings (variant.go, taxonomy.go, links.go, store.go),
// not inside the spine.
//
// The bulk relation loaders at the end live here because they are the product
// listing's N+1 defense; variant.go calls the two leaves of that cluster, but
// the cluster is entered from a product read.
