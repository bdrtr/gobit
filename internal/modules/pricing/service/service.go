...
type Repository interface {
	CreatePriceSet(ctx context.Context, id string, now time.Time) (models.PriceSet, error)
	...
	ListPricesBySets(ctx context.Context, priceSetIDs []string) (map[string][]models.Price, error)
...
func (s *Service) CreatePriceSet(ctx context.Context, prices []PriceInput) (models.PriceSet, error) {
	...
	now := s.clock()
	// Doğrulama YAZMADAN ÖNCE yapılır; geçersiz bir fiyat yüzünden boş bir kap
	// geride kalmaz.
	...
	set, err := s.repo.CreatePriceSet(ctx, models.NewPriceSetID(now), now)
	if err != nil {
		return models.PriceSet{}, err
	}
	...
	if _, err := s.repo.ReplacePrices(ctx, set.ID, toWrite, now); err != nil {
		return models.PriceSet{}, err
	}
	return set, nil
}
...
func buildPrice(priceSetID string, in PriceInput, now time.Time) (models.Price, error) {
	...
	for i, ruleIn := range in.Rules {
		...
		if err != nil {
			return models.Price{}, withIndex(err, i)
		}
