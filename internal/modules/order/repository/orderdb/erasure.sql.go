...
const anonymizeOrderAddresses = `-- name: AnonymizeOrderAddresses :execrows
...
func (q *Queries) AnonymizeOrderAddresses(ctx context.Context, orderIds []string) (int64, error) {
	result, err := q.db.Exec(ctx, anonymizeOrderAddresses, orderIds)
	if err != nil {
		return 0, err
	}
...
const anonymizeOrderContacts = `-- name: AnonymizeOrderContacts :execrows
UPDATE orders
SET email                   = NULL,
...
// the answer stable.
func (q *Queries) AnonymizeOrderContacts(ctx context.Context, orderIds []string) (int64, error) {
	result, err := q.db.Exec(ctx, anonymizeOrderContacts, orderIds)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
const listOrdersForErasure = `-- name: ListOrdersForErasure :many
...
WHERE ($1::text IS NOT NULL AND o.customer_id = $1::text)
   OR ($2::text IS NOT NULL AND o.email = $2::text)
ORDER BY o.id
FOR UPDATE OF o
`

type ListOrdersForErasureParams struct {
	CustomerID *string
	Email      *string
}

type ListOrdersForErasureRow struct {
	ID                   string
	DisplayID            int64
	Status               string
	CurrencyCode         string
...
func (q *Queries) ListOrdersForErasure(ctx context.Context, arg ListOrdersForErasureParams) ([]ListOrdersForErasureRow, error) {
	rows, err := q.db.Query(ctx, listOrdersForErasure, arg.CustomerID, arg.Email)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListOrdersForErasureRow{}
	for rows.Next() {
		var i ListOrdersForErasureRow
		if err := rows.Scan(
			&i.ID,
			&i.DisplayID,
			&i.Status,
