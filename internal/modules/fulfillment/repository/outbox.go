...
//
// The outbox row has to commit with the parcel's own write, and only this side
// is inside that transaction: every module keeps its transaction under its own
...
// does nothing else. It is the same hand the order and payment modules already
// carry, written the same way on purpose — a second shape here would be a second
