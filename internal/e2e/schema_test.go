...
// modules with [describedEndpoints]. The distinction is deliberate: the
// description does not cover every route yet, and an undescribed endpoint is a
// VALID model — it appears in the document with its path, its method and its
...
//
// # Which endpoints are checked, and why not all of them
//
// The audited set is written out below and is checked by hand rather than
// walked, which is the difference between this test and
// [TestUnauthorizedIdentityCanDoNoWorkOnAnyAdminEndpoint] one file over. That
// test can walk the router because its request needs nothing but a path: the
// authorization answer is the same whether the record exists or not. This one
// cannot, because it compares a schema against a REAL RESPONSE, and a real
// response needs a record that really exists — a cart with a line, a product
// with a handle nobody else took. There is no way to conjure one per route
// generically, and a walk that skipped every route it could not seed would
// report a green audit of an empty set.
//
// So the set grows by hand, and it has to cover BOTH SURFACES. Until this test
// also checked an admin endpoint, the whole /admin/v1 tree — every shape an
// operator's console reads — could drift from its description with nothing red
// anywhere: the cart module was the only module whose response was ever
// compared with its schema.
//
// # What the comparison can and cannot catch
//
// Worth stating plainly, because it decides which failures this test is worth
// running for. The schema is generated by REFLECTION over the type named in the
// module's Describe call, so renaming a json tag on that type renames both
// sides at once and passes — that drift is a client's problem and not one this
// test can see. What it does catch is the seam between the two INDEPENDENT
// places that pick the shape: the Describe call names a type, the handler
// writes a value, and nothing in the compiler makes them the same type. A
// describe call left pointing at the request struct, at last release's DTO, or
// at a record the converter no longer produces is invisible to every unit test
// in the module and shows up here as a field set that does not line up.
func TestSchemaMatchesRealResponses(t *testing.T) {
	...
	productID, createdProduct := createSchemaProduct(t)

	t.Run("POST /admin/v1/products", func(t *testing.T) {
		fields, required := recordFields(t, doc, http.MethodPost, "/admin/v1/products", "201")
		actual := responseFields(t, createdProduct)

		mustBeSubset(t, actual, fields, "every field the server writes must be in the schema")
		mustBeSubset(t, required, actual, "every field the schema calls required must be in the response")
	})

	t.Run("GET /admin/v1/products/{id}", func(t *testing.T) {
		recorder, err := adminRequestWithBody(http.MethodGet, "/admin/v1/products/"+productID, nil)
		require.NoError(t, err)
		require.Equal(t, http.StatusOK, recorder.Code,
			"the product must be readable; body: %s", recorder.Body.String())

		fields, required := recordFields(t, doc, http.MethodGet, "/admin/v1/products/{id}", "200")
		actual := responseFields(t, recorder.Body.Bytes())
...
// createSchemaProduct creates a product from the ADMIN endpoint and returns its
// id together with the raw creation response.
//
// It goes over HTTP rather than through [newVariant]'s service call on purpose:
// what is being compared with the schema is the body the HANDLER writes, and a
// record made through the service would leave the create response — half of the
// comparison — unproduced.
func createSchemaProduct(t *testing.T) (productID string, body []byte) {
	t.Helper()

	seq := fixtureCounter.Add(1)
	recorder, err := adminRequestWithBody(http.MethodPost, "/admin/v1/products", map[string]any{
		"handle": fmt.Sprintf("e2e-schema-product-%d", seq),
		"title":  "Schema Audit Product",
		"status": "published",
	})
	require.NoError(t, err, "could not build the product request")
	require.Equal(t, http.StatusCreated, recorder.Code,
		"the product must be created; body: %s", recorder.Body.String())
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &envelope),
		"could not decode the created product; body: %s", recorder.Body.String())
	require.NotEmpty(t, envelope.Data.ID, "the created product must carry an identity")

	return envelope.Data.ID, recorder.Body.Bytes()
}
