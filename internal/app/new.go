...
//
// A plain `go build` or `go install` leaves them empty, and then the version the
// toolchain stamped into the binary answers instead ([scaffold.Stamped], ADR
// 0182). The refusal below is for a binary that has neither.
var (
	buildRelease    string
	buildBaseTag    string
	buildCommit     string
