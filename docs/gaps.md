...
| A2 | ~~**What gobit is legally** — data controller, or a library whose embedder is the controller~~ **Answered 2026-09-06: ADR 0029.** The embedder is the controller. gobit owes the MECHANISM and not the policy: an erasure contract whose outcomes are DELETED, ANONYMIZED and RETAINED; hooks so a module can say a person was erased (B8); and a declaration of what each module holds. It does not owe the retention period, the lawful basis or the consent text | every KVKK item; ADR 0025 points at the second answer, which changes the obligation from "implement consent" to "publish the hooks and the erasure contract" | Turkey-specific |
| A3 | ~~**May the customer pay a different amount than the merchant receives?**~~ **ANSWERED 2026-09-07: ADR 0042 — not now, and when it comes it arrives as a SETTLEMENT ROW rather than by relaxing the equality.** The equality is held in layers and the ADR names each; the two the measurement corrected are worth carrying: `orders.total` and `payment_collections.amount` are not two numbers that happen to agree but ONE number written twice (the plan's Amount), and the ceiling is a database CHECK while the floor is in the saga. **What the adversarial pass overturned:** the claim that no existing money table could hold a settlement row is FALSE — `payments_session_uniq` is per SESSION, `payments_collection_idx` is non-unique, and a rolled-back probe inserted two payments summing to 1100 against a collection of 1000 and all five statements were accepted. The multiplicity a settlement row needs already exists; what is missing is a kind column, an ALTER rather than a new table. **The one-directional residue:** `POST /admin/v1/payment-sessions/{id}/capture` is bounded only above and `payment_collections` has a first-class `partially_captured` status, so an operator can already make the merchant receive LESS than the order total outside the saga — "forced EQUAL" was never true downward | installments (vade farki) AND multi-vendor commission. Today four guards including DB CHECKs forbid it | Turkey-specific, Commerce models |
| A4 | ~~**Invoice retention vs KVKK erasure**~~ **Answered 2026-09-06: ADR 0032 — an issued invoice refuses erasure and the refusal lives in the SCHEMA, because a refusal in Go stops the caller while one in the database stops the STATEMENT. The contract returns RETAINED. ~~HOW the schema enforces it is left open in the ADR and deliberately so: a BEFORE DELETE trigger carries a message but would be this repository's FIRST procedural code in a migration, while `REVOKE DELETE` introduces no new kind of code and no message.~~ **The sub-question was answered on 2026-09-07 by measurement, not by weighing.** `REVOKE DELETE` does NOTHING here: gobit's shipped role is a superuser, the revoke succeeds, `relacl` visibly changes, and the next `DELETE` still reports `DELETE 1` — a guard that reports success and stops nothing. So migration 000002 carries four triggers (DELETE and TRUNCATE, on `invoices` AND on `invoice_lines` — with `invoices` guarded alone, `DELETE FROM invoice_lines` still succeeded and orphaned the totals), a custom SQLSTATE rather than 23001, and `ENABLE ALWAYS` — without which one `SET session_replication_role = replica` silenced every trigger. Measured both ways. The remaining limit is written down rather than papered over: the trigger does not stop the role that OWNS the table, which is a role-separation gap filed against ADR 0015. The two free-text fields get a published rule rather than validation — an embedder that puts personal data in `metadata` or a line's `description` retains it under the same refusal.** Original naming below.** **When a person asks to be erased and they are the buyer on an ISSUED invoice, what does the invoice module do with what it holds about them — refuse, blank it in place, or let it expire on a clock — and does the answer bind the two FREE-TEXT fields as well as the six named ones?** Measured: `invoices` carries six `buyer_*` columns and, on the same row, a `metadata` jsonb the caller fills and nothing validates; every `invoice_lines.description` is caller text too. Nothing can delete an invoice today and the protection is the ABSENCE of code rather than a constraint — the module's queries, its repository and its five routes contain no DELETE at all, so an erasure implementation issuing one would succeed. ADR 0024 says a canceled document keeps its number and stays in the table. Candidates: **(1) refuse, and leave the refusal a policy** — costs nothing in the module, and pushes a third outcome onto whatever contract B17 builds; **(2) refuse, and put the refusal in the SCHEMA** — the only candidate that closes the live hazard, because it stops the statement rather than the caller; **(3) keep the number and the amounts, redact the person in place** — supersedes ADR 0024's "no update path for its parties" clause and must say what happens to the two free-text fields; **(4) keep it whole for a window, then expire the whole (prefix, year) cohort** — the only one that needs a retention setting, and this repository has none outside the two idempotency stores. **A2 is upstream of this row and the row did not say so until 2026-09-06**, which is the defect A15 was corrected for: under the first answer gobit picks the retention policy, under the second it publishes the mechanism and the embedder sets the window, and those are different candidates rather than different wordings. B17 needs both answered and says so. Every candidate needs B8 as its mechanism: `DeleteCustomer` is a soft delete and the customer module publishes NO event, so nothing can learn an erasure happened | all erasure work; and the hazard is live rather than hypothetical — nothing in the schema stops a `DELETE FROM invoices` | Observability and security |
| A5 | ~~**Group-price selection for a customer in several groups**~~ **ANSWERED 2026-09-07: ADR 0049 — ONE group decides, and the merchant ranks the GROUPS. BUILT 2026-09-08, all five parts.** A merchant-set rank on `customer_group` (O(groups), not O(customers)); `CustomerGroupIDs` keeps its frozen signature and its ORDER becomes contractual; the cart sends the highest-ranked group the customer belongs to. **A preliminary decision said "send the SET and let the existing ladder decide unchanged", and an opposition pass DEFEATED it on three measurements.** (i) The ladder's merchant lever is exhausted at two ranks — `price_list.type` admits only `sale` and `override` — so two contract prices on two different override lists tie at every earlier rung and are decided by AMOUNT at rung four: "send the set, ladder unchanged" IS cheapest-wins in the one configuration A5 is about, which the preliminary decision had explicitly refused. (ii) It inverts the ladder's own philosophy, that a higher-ranked SOURCE wins even when a lower-ranked one is cheaper. (iii) "No new mechanism" was false: a rule context is `map[string]string` and `matchRule` compares ONE context value against the rule's value list, so "any of my groups" is not expressible; the pricing HTTP surface rejects a repeated parameter with the invariant written in its godoc; and the SET lives on the RULE while the single value lives on the CONTEXT, so sending a set inverts the direction the storage was designed around, in three independent copies of the matcher. The repository had already named the real blocker — the cart package doc says the obstacle is the absence of a DETERMINISTIC choice, "silently picking one would tie the price to map iteration order". **The residue is recorded rather than argued away:** two prices carrying the SAME group rule still meet at rung four, so cheapest-wins is NARROWED and not removed | B2B group pricing | Commerce models |
| A6 | ~~**`allow_backorder`: give it a reader or stop publishing it**~~ **ANSWERED 2026-09-07: ADR 0048 — all four get a reader:** the stock pair in the checkout saga, the product pair in the campaign engine. **BUILT 2026-09-08, both halves.** The stock pair rides on the variant read the checkout already makes for the title (one more `Fields` entry, zero extra round trips): `manage_inventory` decides whether a line is reserved at all, `allow_backorder` decides whether a line no warehouse can cover — including one linked to no inventory item, which no warehouse can EVER cover — refuses the order, and the plan carries the first as its INVERSE (`Unmanaged`) so a record written before today decodes as counted. That closes the badge-versus-till disagreement on both of the clauses ADR 0040 spends these columns on, which is the thing the decision was written for. The product pair goes into the cart's discount request as two line attributes and the promotion module was not touched, exactly as measured; what turns them into a decision is still a `promotion_rule` row a merchant writes, and that residue is the record's first negative. **Two corrections the check pass forced into the record, both of the class this week is about:** the draft credited `TestEveryColumnIsWrittenBySomething` with proving the boolean columns are written, and that gate explicitly DISCLAIMS it — `auditColumns` skips any column the database supplies, every one of these is `NOT NULL DEFAULT`, so none is inside that audit and it would stay green if one stopped being written. The fact is true and is now stated directly, checked column by column against the query files. The second: the cost of hoisting the variant-to-product hop was stated as ONE additional batch Graph unconditionally, when the tax leg pays it on only one of three paths — the record now gives the cost per tax path | ~~pre-order; today a published flag does nothing, and the flags measurement names it as the live proof of ADR 0009's second error class~~ **all four flags decide something as of 2026-09-08, so the live proof is spent; pre-order is what `allow_backorder` still cannot MEAN — no negative level, no promised date — and it is a decision of its own** | Commerce models |
| A7 | ~~**Does the panel become an admin-API client?**~~ **Answered 2026-09-06: ADR 0030, amending ADR 0011.** Yes — an SPA served from the binary, authenticating with the SAME HttpOnly cookie, so ADR 0011's refusal of localStorage stands. The cookie's Path widens to cover `/admin/v1` and the ADR writes the price down rather than calling it a wash: today's immunity comes from the cookie never being SENT there, and after this it comes from `SameSite=Strict` plus `UI.CheckOrigin` — two guards that already exist, one layer thinner than three. Whoever implements it owes the Origin guard on the API prefix, a rule that does not refuse `sk_` callers, and a JSON refusal instead of the panel's HTML error page | the SPA, and the extension-point design | Admin panel |
| A8 | ~~**Catalog cacheability**~~ **ANSWERED 2026-09-07: ADR 0044 — the sales channel moves into the PATH,** and the publishable key becomes a gate rather than an input to the answer. **BUILT 2026-09-08, THREE of the four routes:** the listing, the single product and the option vocabulary now live under `/store/v1/sales-channels/{sales_channel_id}/`, narrow to the named channel and refuse with 403 a channel the key does not hold; ~~the fourth, the search plugin's `GET /store/v1/search`, has NOT moved and still takes its channel from the identity alone~~ **and the fourth moved 2026-09-08 too, together with the audit the record said it was owed: `GET /store/v1/sales-channels/{sales_channel_id}/search`, narrowing through the same helper as the other three. A8 is now BUILT IN FULL.** That is what makes a shared cache possible at all: ~~today the channel arrives out of band, so one storefront's catalog and another's are the same URL with different answers~~ **until the build the channel arrived out of band, so one storefront's catalog and another's were the same URL with different answers — on the three moved reads they are now two URLs and two keys authorized for one channel receive byte-identical bodies, and on search they are still one URL**, which is the thing the GraphQL handler's own godoc says a shared cache cannot store | edge caching. The repo already argued the default answer in the GraphQL handler | Storefront speed |
| A9 | ~~**Customer identity: does ADR 0008 stand or is it superseded?**~~ **ANSWERED 2026-09-07: ADR 0043 — it STANDS. BUILT 2026-09-08.** gobit requires an identity at the address book and still does not issue one: `corehttp.Identity` is published, the customer module resolves it from the container under `corehttp.IdentityName` on first use, and the eight storefront routes naming a customer refuse a claim it does not back — 403 on a mismatch, 401 when nothing is bound. The trust boundary ADR 0008 drew is unchanged; what changed is that the embedder now has a NAMED seam to satisfy it, and that the failure mode of never satisfying it is a refusal instead of a leak. The CART is untouched: ADR 0008's measurement reproduces exactly | customer login, passkeys, a customer-facing assistant, A/B assignment, and it is why the address book was unauthorized for the customer — it was never unauthenticated, the publishable key is checked in front of it | Storefront speed |
| A10 | ~~**pgvector: reopen ADR 0015 or not?**~~ **ANSWERED 2026-09-07: ADR 0045 — NOT reopened.** The cluster contract keeps its "extensions: none" promise for the framework, and pgvector arrives as a separate OPT-IN extension module that an installation chooses, the way `searchpg` is chosen. An installation that does not install it meets no new cluster requirement | semantic search, visual search, embedding recommendations | AI subsystem |
| A11 | **Where translated content lives** — **gobit's POSITION is decided (ADR 0050, 2026-09-07): it stores ONE language and the second is the embedder's. The GAP itself stays OPEN, and the record says so in its own words: "It does not close gap A11. The question stands; what this record fixes is the false premise it was about to be closed on."** The deferral carries a trigger rather than a shrug: ADR 0025 reopens a boundary decision when the first customer projects show a surface they genuinely cannot express, and what they would have to show is WHICH demand is real — a per-DEPLOYMENT language (two installations, one language each, which already works) or a per-REQUEST language (one installation, the shopper picks). Only the second needs storage. Two further facts make the timing wrong rather than merely unproven: no locale reaches a handler gobit owns, and ~~ADR 0044 is Accepted and unbuilt while it rewrites the very catalog read paths a locale would ride — deciding locale delivery into routes already scheduled to move decides it twice~~ **(2026-09-08: ADR 0044 is BUILT on three of its four routes, so that half of the timing argument is spent. What is left of it is the fourth, the search plugin's `GET /store/v1/search`, which has not moved; the deferral now rests mainly on the first fact, that no locale reaches a handler gobit owns.)** **(Later the same day: the fourth route moved as well, so that half of the timing argument is spent in full and the deferral rests entirely on the first fact.)** Candidate (4), the one consistent with ADR 0025. The finding that reorders the question stands: the storefront has no notion of a shopper's language at all, so the first question is not where a translation lives but how a request asks for one, and nothing answers it — a translation table nothing can key into is a table. **What the opposition pass corrected is a claim that would have made this decision dishonest:** the draft said gobit "does not PREVENT an embedder from adding translations in their own module". Measured, it does. `internal/app.Options` carries three fields and no middleware hook; the router's middlewares are filled once from gobit's own guard stack; `core/plugin.Host`'s fourteen methods install none (`AddRoutes` adds, it does not wrap); and the plugin registry REFUSES a route collision at startup with `plugin_route_conflict`, so an embedder cannot re-serve `GET /store/v1/sales-channels/{sales_channel_id}/products` with a translating handler either. The cost is stated in the record rather than softened. Candidate (3), one translation module keyed by (entity, id, field, locale), is the strongest storage answer and is the one that should win IF the language question is ever answered | multi-language, and it touches the search index, the panel and the invoice | Platform features |
| A12 | ~~**JWT TTL policy**~~ **Answered 2026-09-06: ADR 0031 — a fixed twelve hours, bounded in shared environments and documented where an operator reads it. BUILT 2026-09-07, all three obligations: `config.MaxSharedJWTTTL` (24h, shared only, mutation-proved at both edges), the row in both READMEs plus `.env.example` and `docs/security.md`, and the message.** ~~The ADR records one defect it accepts: a naturally expired session sends no cookie, so the panel takes its `token == ""` branch and shows a login page with an EMPTY message.~~ Closed — and closing it corrected the ADR: that branch is reached by TWO people (a lapsed session and a first visit), so the fix could not be "a message" as the ADR claimed. It needed state outliving the credential, and that is a second cookie carrying no credential (`gobit_admin_seen`). The page the operator was interrupted on is now carried through the sign-in as well; its open-redirect guard had a dead branch hiding a live hole (`/admin/ui/../../admin/v1` was ACCEPTED by the first draft) and now parses rather than string-matches. Original naming below.** **Does gobit RATIFY the fixed lifetime (publishing the trade and bounding the value), or add RENEWAL — and if renewal, is the renewal credential a separate revocable row, does the live token renew itself at one identity endpoint, or does only the panel renew?** Measured: the lifetime is `JWT_TTL`, twelve hours by default, and `Config.Validate` accepts any positive value at all. The half everyone expects to be missing is already BUILT: verification is not merely cryptographic — it reads the user and then a session anchor on every request, and refuses a token issued before a logout or a password change. So revocation exists and is coarse, which makes "a long lifetime plus revocation" a configuration act rather than a build, and makes the refresh answer the expensive one. Candidates: **(1) ratify the clock** — no table, no endpoint, no ADR superseded; the price is that the mid-task logout stays; **(2) a separate refresh credential** — the only candidate needing both a new table and a new route, in a module whose one migration already holds five tables; **(3) re-issue the LIVE token at one identity endpoint** — no table, and the token becomes its own renewal credential; **(4) the panel renews silently and the JSON surface keeps the raw clock** — no published surface touched, and it leaves two session models in one product. Whichever is chosen, the answer has to carry a NUMBER, because A7's auth story turns on the words "short-lived" | session design; and A7 cannot be written until this carries a number | Observability and security |
| A13 | ~~**Metrics posture**~~ **ANSWERED 2026-09-07: ADR 0046 — metrics leave by SCRAPE, and OTLP keeps the traces. BUILT 2026-09-08.** Measured while deciding it, and fixed in the same round: an empty `OTEL_EXPORTER_OTLP_ENDPOINT` turns off ALL telemetry rather than only tracing, because `observability.Setup` returns before building either provider — so in the default install there is no meter provider at all and the two instruments record into no-ops. `config.go` said "tracing is turned off entirely" and this table said flatly that a meter provider and an exporter exist; both are corrected. **What the build then changed about that measurement:** the early return in `observability.Setup` widens to BOTH signals, so an empty `OTEL_EXPORTER_OTLP_ENDPOINT` turns off the TRACES alone; the meter provider is built from a Prometheus PULL reader whenever `METRICS_ADDR` is set and served at `/metrics` on a listener of its own; and the periodic OTLP metric exporter left the tree together with `METRIC_EXPORT_INTERVAL`. A stock installation still has no meter provider, because `METRICS_ADDR` is empty by default — but now that is a switch left off rather than a signal with no switch. **Also unremarked until now:** no dependency allowlist exists — `depguard_test.go`, despite the name, audits only the module-isolation matrix, so a new client library trips no arch test | the "measurable speed" claim | Observability and security |
| A14 | ~~**Price history: promote the accidental retention or drop it?**~~ **ANSWERED 2026-09-07: ADR 0047 — DROP it, and what survived a replace was never a history. BUILT 2026-09-08, and the schema did not move.** `ReplacePrices` calls a `DELETE` carrying the same liveness predicate the stamp carried, so the tidy-up still plans as an index scan on `price_set_id_idx`; the rules go with the price through the `ON DELETE CASCADE` that was already there. `price.deleted_at` STANDS and now has exactly one writer, `DeletePriceSet` — the only thing hiding a deleted set's prices from a calculation, since `ListPriceCandidates` does not join `price_set` — and the schema file's header, which used to say deletion in this module is soft module-wide, was corrected to name the exception. Two costs are carried rather than closed: the rows an older version of the code already stamped are outside the predicate and stay until somebody writes the operator cleanup this record deliberately did not commission, and "what did this variant cost last month" is now permanently unanswerable. The adversarial pass corrected the shape of the risk: removing the column does not fail silently — the module's own read returns SQLSTATE 42703, `column "deleted_at" does not exist`, immediately and totally | forecasting | AI-powered features |
| A15 | ~~**May the storefront accept content from a party it cannot identify?**~~ **ANSWERED 2026-09-07: ADR 0051 — only when the write is CONFINED or INERT, and the rule is held MECHANICALLY.** The discriminator is ratified from the order module's own store-handler argument, and the opposition pass replaced the preliminary wording: "a HUMAN stands between the write and its effect" compresses two clauses into one and loses the half that does the work. CONFINED — the effect completes inside the writer's own reach; INERT — the write moves no stock and no money and every endpoint that ACTS on it is admin-only and scoped. **The second constraint also changed, and its reason is the decisive finding:** the preliminary said the rule must be held by the TYPE, and the type-level guarantee DRIFTED inside the worked example within one release — so a rule held by types and prose alone demonstrably does not hold, and what the record requires is a gate that fails when the property stops being true. Unchanged: the discriminator constrains the SCHEMA and not only the flow, and the back-in-stock waitlist (C1) FAILS the test as specified, because the effect of a waitlist row is an outbound message nobody approves. **The gate is BUILT in all three parts, the last of them on 2026-09-08.** The write and the read boundaries are held by `internal/arch/storefront_boundary_test.go` — no storefront write decodes an operator-controlled field, no storefront read offers one as a query parameter — and the SCHEMA clause by `internal/arch/storefront_schema_test.go`, which is the route-to-table cross-reference the record said nothing in this tree performed. Every storefront write route **under `internal/modules`** — twenty-two of the twenty-five; `plugins/webpush` mounts its three behind a route PREFIX the scan does not follow and they stay on the open-defect row — resolves to the tables it can store into: the module it was registered in, the call graph from its handler through `internal/workflows` to the SQL, and an intersection with the tables that module's own migrations create; then every column of every reached table is read for the four claims the review module's refusals define — a party, a contact, a network origin and a prior record of the shop. **The intersection's cost is stated rather than left to be found:** a write another MODULE performs on a route's behalf is not that route's, so the checkout saga's own stored claims sit outside — `orders.customer_id` and `orders.email` measured UNREACHED on 2026-09-08, and they are where the cart's declared `customer_id` lands. A claim with no verdict fails the gate, a verdict for a claim that has gone fails it the same way, and a party column the storefront BODY carries may NOT be recorded under EITHER limb: not CONFINED, because that limb says no subject the client names may be a party the writer does not already hold, and not INERT, because both limbs judge the WRITE while this clause judges the COLUMN — the INERT half was missing until 2026-09-08, when rewriting the cart's row as INERT was measured GREEN. Eight claim columns are reachable today; the cart's `customer_id` is the one recorded as an OPEN DEFECT with its closing rows rather than exempted. **One measurement changed the build:** a walk stopping at the module boundary resolved FOUR of the cart's eleven storefront writes to no table at all, because the cart opens, prices and completes through narrow interfaces the composition root binds — so a route that reaches no table is now itself a failure | the back-in-stock waitlist (C1), ~~the review module (B4)~~ built, and every future "tell me when" or "write us something" feature; A2 sits upstream of it | Commerce models |
| A16 | ~~**What amount would a price filter compare against?**~~ **ANSWERED 2026-09-07: ADR 0041.** The request's currency, the BASE price, quantity tier one, no customer-group context, evaluated at the moment of the query; where a product has several variants it matches if any variant's base price does. **The measured correction:** "the default price list" is not a list. `price_list.type` is constrained to `sale` or `override` and there is no `is_default` column anywhere in pricing — what exists is `price.price_list_id`, which is NULLABLE, so the base price is the ABSENCE of a list and the predicate is `price_list_id IS NULL` rather than a join. Written down so nobody implements this by adding a default flag. Rejected: the lowest price across lists (the filter would move when a sale opens, with nobody editing anything, and the minimum cannot be indexed against a stable column), the promoted price (undefined outside a cart), a precomputed column (a window-bounded list opening at midnight leaves every stored amount wrong), and currency conversion (a rate is a fact about a moment this repository does not store). **THE FILTER WAS BUILT 2026-09-08; ITS INDEX WAS NOT, and that is the measurement the build adds.** `currency_code`, `min_price`, `max_price` on REST and one `price: PriceFilter` input object on GraphQL, validated ONCE in the service so the two surfaces cannot drift into accepting different requests. "It is indexable" is true of PRICING and not of the caller: the filter runs in the CATALOG, which may not join pricing's tables and has no predicate to push down (pricing's Query provider accepts exactly one filter, `id`), so the comparison happens in Go over records the request already fetched and rides ADR 0040's scan with the same three costs. **The second cost is the one to carry away:** the predicate needs SIX names out of pricing's loose record, the road ADR 0040 opened for ONE inventory field and refused to widen -- and pricing declares all six as UNEXPORTED constants, so the audit cannot bind them. They are registered as a known hole with a test that fails the day pricing exports them; until then a rename inside pricing silently empties a price-filtered catalog and only the end-to-end test, which prices its fixture through the real pricing module, would catch it | the price filter AND the price sort in B2; "under 500 TL" in C10; a price column in any search index | AI-powered features |
| A17 | ~~**What does "in stock" mean for a PRODUCT?**~~ **ANSWERED 2026-09-07: ADR 0040.** A VARIANT is in stock when `manage_inventory` is false, OR `allow_backorder` is true, OR its available quantity exceeds zero; a PRODUCT is in stock when at least one variant is, and a product with no variants is not. A variant that is counted but linked to no inventory item is NOT in stock — the alternative sells what the shop may not have. Region is out of v1 deliberately: `available_quantity` is the total across ALL locations, and a regional answer needs a locations-serve-regions decision nothing here asks yet. **The measured correction:** the answer cannot be split "variant to inventory, aggregation to catalog", because `inventory_items` carries neither flag and may not read product's models (Principle 2.1) — inventory cannot answer for a variant at all. The three inputs meet in exactly one place, `StoreVariant`, where the catalog already holds both flags and receives the linked `inventory_item` with `available_quantity` through the Query layer. So both halves are the catalog's and inventory keeps the quantity. **The accepted price, stated in the ADR:** the catalog must read ONE published field name out of another module's loose record, which `StoreVariant`'s godoc says is deliberately not done — and the two literals that must agree sit in different modules with nothing comparing them, ~~so that pairing needs an audit that does not exist yet~~ **BUILT 2026-09-08 -- badge, filter AND the audit.** The badge is `in_stock` on every storefront product and variant, written unconditionally because the inputs are already fetched; the filter is `in_stock=true|false` and answers both directions. The audit is in `internal/arch/provider_fields_test.go` beside the panel's, with the same two halves: the catalog's one declared dependency is checked against what inventory still publishes, and the catalog's own source is READ so an undeclared one fails -- which is what turns "does not license a second field" from a sentence into a gate. **The build found a cost the record did not anticipate, and it is about the PAGE:** the answer exists only after enrichment and enrichment happens after `LIMIT` cuts the page, so filtering what came back would fill the page short and, on a sparse match, produce an EMPTY page in the middle of the catalog that a client cannot tell from the end. The listing SCANS instead -- chunks of a hundred, one batch Graph call each, a budget of five hundred rows -- and pays for it three ways: a short page with a cursor is possible, an offset is REFUSED beside it, and the listing is not counted (an explicit `with_count=true` is refused rather than answered with a missing field) | the in-stock filter in B2; real-time stock (C16) reads the same absence; the waitlist (C1) needs the same definition to know when to fire | AI-powered features |
| A18 | ~~**When a shopper filters by "Color: red", what counts as a match?**~~ **ANSWERED 2026-09-07: ADR 0039 — the third candidate, with one correction that came out of measuring it before building it.** The value is stored twice: `value` as the merchant typed it and `value_folded` as it is matched, with a UNIQUE index per option on the folded form. The fold is GO's, never the cluster's (ADR 0038's finding is one day old). **What changed from the candidate as written:** A18 proposed folding "the text folded to ASCII the way a handle already is", i.e. `slugify` — and `slugify` DROPS any rune it cannot transliterate, which is right for a URL handle and destroying for a matching form. Every Cyrillic value in a catalog folds to the EMPTY STRING, as does every CJK one, so they are all equal to one another; and because the folded form carries a unique index, the second such value is not mismatched but REFUSED. Reproduced on the real schema: two Cyrillic colours in one option fail on `product_option_value_folded_uniq` with `Key (option_id, value_folded)=(o2, ) already exists` — **a catalog written in Cyrillic could not run the migration at all.** The rule is flawless on Turkish, which is why it read as safe. `models.FoldOptionValue` therefore trims, lower-cases with Go's Unicode rules, folds Latin marks to their ASCII base, and KEEPS every rune it has no base for: two spellings of one value meet, two different values do not, and non-Latin scripts gain case-insensitivity instead of losing their text. The startup convergence pass was built in the same round (`refoldOptionValues`, beside the invoice module's), and it answered a case the decision had not anticipated: on a C-locale cluster the migration SUCCEEDS and the convergence then collides on the unique index, because the SQL backfill left two spellings of one word apart and the Go fold brings them together — so the pass reports each collision at ERROR with both forms rather than choosing between them. ~~Still open on top of it: the filter itself~~ **THE FILTER WAS BUILT 2026-09-08.** `option_value` on the REST storefront listing and `optionValue` on the GraphQL one; the service folds the caller's text ONCE on the way down and the repository compares two STORED strings in an `EXISTS` over `product_option_value` joined to `product_option`, so the page and the count are both over the filtered set. **The build corrected the record's own claim that it had already decided the index:** `product_option_value_folded_uniq` leads with `option_id` and a catalog filter has no option id, so it can serve IDENTITY and not MATCHING -- migration 000004 adds `product_option_value_folded_idx`, partial and non-unique, and the values become the driving relation instead of the catalog. The clause guards the soft delete on BOTH parents, because deleting an option is not a cascade and its values stay live. Proved against a real database (two spellings of one word meeting, a third value staying apart, a value under a deleted option refused) and end to end through the vocabulary endpoint, which is what a storefront actually does: click a value, send it back | B2's option-value filter, and C10 through it | AI-powered features |
...
seventeen open rows the same day (A18, out of B2's option-value
...
**In what order should these be answered? Measured 2026-09-06, and the answer is
mostly "any", with one root and one PAIR.**

The group reads as a list of seventeen independent questions and it very nearly
is. Of the seventeen rows this paragraph was written for, eight carried a
recorded relation and nine carried none, and only three ordering facts were
written down anywhere in this file. **Four of those seventeen were answered on
2026-09-06 — A2, A4, A7 and A12 — so thirteen remain open and the first two
facts below are now HISTORY rather than instructions.** They are kept because
they are the argument for why the four were answered in that order.

- ~~**A2 is the root.**~~ **Answered — ADR 0029.** It was the only row two others named as upstream: A4's
  candidates change shape depending on it (gobit picks the retention policy, or
  publishes the mechanism and the embedder sets the window), and B17 says "A2
  and A4 come first" of the whole erasure body of work. A15 records the same
  relation for the class it describes. Answering A2 costs nothing today and
  changes the OPTIONS of the rows below it, not merely their priority.
- ~~**A7 and A12 are a PAIR and not a sequence.**~~ **Both answered in one sitting — ADR 0030 and ADR 0031, which is what this fact asked for.** Each one's answer was an input to
  the other: A7's auth story turns on the words "short-lived", which is exactly
  the number A12 has not fixed, and one of A12's candidates — the panel renewing
  silently — is void if A7 makes the panel an API client. Answering either alone
  leaves the other half-answered, so they belong in one sitting.
- **A16, A17 and A18 are siblings, not a chain.** All three came out of B2's
  filter surface on different days, and none is upstream of another: a price
  filter, an availability filter and a text-matching rule can be decided in any
  order or separately.

The nine that name no upstream — A3, A5, A6, A8, A9, A10, A11, A13, A14 — are
answerable on their own. A5 is worth one word of that: it is CONSTRAINED by an
accepted ADR rather than by another open row, and a decision already made is not
an ordering relation. That is not an accident of the writing: a decision whose
answer depends on another decision is the one thing this table has to say out
loud, because a reader answering them in the wrong order pays for the same
question twice, which is the rule A15's correction added.
...
| B8 | ~~**Customer module events** (`customer.deleted` at minimum)~~ **NOT DELIVERED, and deliberately — the obligation it stood for was met a different way on 2026-09-06 (ADR 0033).** This row existed because nothing could learn that a person had been erased. Measured while building B17, the question turned out to be the wrong one: ADR 0029's obligation is not that a holder LEARNS about an erasure, it is that the CONTROLLER gets an answer — an outcome per holder with a reason attached, because somebody has to reply to the person who asked. An announcement carries no reply, and three measurements closed the option rather than ranking it. `core/eventbus`'s own package doc records that `Publish` does not wait, that a handler's error never reaches the publisher, that the in-memory backend is at-most-once and loses the event if the process dies, and that no backend retries — so an event-driven erasure cannot report what it did. `TestTheEventTopicsHaveASubscriber` is satisfied by the FIRST subscriber it finds, so a `customer.erased` topic with one listener would be green while every other holder of personal data never heard it; the gate cannot make the event honest. And firing it from `DeleteCustomer` would have made gobit choose POLICY: a shop deleting a customer record is bookkeeping, an erasure is a legal answer, and ADR 0029 puts that judgement with the embedder. **What shipped instead is the hook ADR 0029 actually asked for** — the optional `erasure.Eraser` interface, found by type assertion over the module registry, which returns an answer. The customer module still publishes no event, and that is now a decision rather than a gap | ~~erasure~~ — nothing. The row it blocked is built | Turkey-specific |
| B9 | **Stored payment instrument** — provider contract + table, and the row named neither of its two blockers until 2026-09-06. Measured: there is no trace of a stored instrument anywhere — not a table, not a column, not a symbol in the payment module or in `core/provider`. Both halves are decisions this row cannot take on its own. The CONTRACT half is B10's blocker exactly: `PaymentProvider` lives in `core/provider`, one of the fourteen packages ADR 0026 published, so widening it is a compatibility decision rather than an edit — and a provider that cannot vault an instrument cannot be asked for one. The CONSUMER half is B7's and B13's rule: a stored instrument with no checkout path that spends it is a capability with no consumer, and this repository has twice decided that a capability and its first consumer are one package or neither | saved cards AND subscriptions, in one change | Storefront speed, Commerce models |
...
| B15 | **File read-back, ~~product-image ↔ upload link~~ built 2026-09-05**, file events still missing — and they carry B7's blocker, which this row did not say until 2026-09-06. Measured: the file module publishes NO event at all, so this is not a topic that needs a second subscriber, it is a module that has never spoken. `TestTheEventTopicsHaveASubscriber` refuses a topic no production file listens to and its exemption map is empty by policy, so the first file event and its first listener are one change or neither. `product_image.upload_id` plus the `upload_product_image` link (declared by PRODUCT — it writes the record the binding carries; the file module's own doc says it does not know what a file belongs to). The file module gained the interop surface it deliberately lacked, and the reasoning behind that absence turned out to be half wrong: the address shows the file and says nothing ABOUT it | anything that looks at a photo — the id half is answerable now; file EVENTS are not built | AI-powered features |
| B16 | **A suggestion store** — system proposes, human applies. Measured 2026-09-06 and the row's own sentence understates it: no table named for a suggestion exists in any migration, and the two ENDS are missing as well. Nothing would write one — every feature that proposes is an AI row and none is built — and nothing would read one, because the panel has no screen for a human to apply or dismiss. `sagawatch` is the cited pattern and it is a good one, but what it shows is a job that REPORTS; the shape this row wants is a job that WRITES and a screen that answers | forecast and category suggestions. The pattern exists (`sagawatch`, ADR 0017); the storage does not | AI-powered features |
| B17 | **KVKK erasure, export and retention** — ~~A2 and A4 come first~~ answered 2026-09-06, ~~then B8~~ **and the ERASURE third is BUILT the same day (ADR 0033); export and retention are not.** Built: `core/personaldata` as the fifteenth published package (ADR 0026's amendment carries the argument — an embedder's own module must NAME the contract to implement it, and Go's internal rule makes that impossible from `internal/`); a synchronous sweep in `internal/workflows/datasubject` that walks `module.Registry.Modules()` by type assertion, so gobit's modules, the ones plugins bring and the ones the embedding application adds are all reached by one walk; `POST /admin/v1/erasure` and `GET /admin/v1/erasure/personal-data` bound at the COMPOSITION ROOT rather than on the customer module, because the subject need not have a customer record at all — `invoices` has no `customer_id` and no `order_id`, and a guest's order carries an `email` with a NULL `customer_id`. Three modules answer: customer ANONYMIZED, order ANONYMIZED with a RETAINED carve-out while the order is unsettled, invoice RETAINED with the refusal now in the SCHEMA. **The rule that keeps the word honest:** gobit never rewrites a free-form column, so every ANONYMIZED result NAMES the open columns it left. **NOT built, and named rather than omitted:** data EXPORT (portability) — nothing in the tree assembles what is held about a person into a document; a retention WINDOW, which under ADR 0029 is the embedder's number; ~~and the pruning of `workflow_executions.input`~~ **built 2026-09-07: the saga store erases its own rows and answers ANONYMIZED.** It EDITS the input rather than dropping it — `email` emptied, both addresses nulled, everything else kept — because the one thing that genuinely reads that column is `gobit recover`, which rebuilds a half-finished saga's compensation chain from it and refuses a plan with no cart id. No status filter: measured, nothing reads the input on any path except that command, there is no `IsTerminal` in the engine, and `status` is free text with no enum, so "terminal" would have been a hand-kept list in Go | ~~a legal requirement~~ the erasure third is met; C4 (data-subject endpoints) now has a contract to call | Turkey-specific |
...
**Every one of the eight open rows in this group is BLOCKED, and every blocker
is one of two things: a DECISION nobody has made, or a CONSUMER that does not
exist.** The decision kind comes in two flavours, which is why the list below
has three bullets rather than two. It was measured on 2026-09-06, and only after
FOUR of the eight — B8, B9, B15 and B16 — were found to be carrying a blocker
their own row did not name:

- ~~**Waiting on a decision in group A:** B2 (its option-value half on A18, its
  price and availability halves on A16 and A17).~~ **B2 is no longer waiting on
  group A: all three were answered on 2026-09-07 and built on 2026-09-08.** ~~and B17 (A2 and A4)~~ —
...
- **D2** ~~`allow_backorder` is published and does nothing (see A6).~~ **CLOSED 2026-09-08: all four now have a reader (ADR 0048) — the stock pair in the checkout saga, the product pair in the campaign engine. The sentence below is kept because what it MEASURED is still the reason the flags were safe to leave unread while A6 waited.** **Measured
...
- **D3** ~~The address book's storefront endpoints are unauthenticated and keyed
  by a path id — personal data anyone can read and change. A known consequence
  of ADR 0008, in its sharpest form.~~ **Closed 2026-09-08 by ADR 0043, and the
  word "unauthenticated" was wrong the whole time.** A request with no
  publishable key never reached those handlers; it took a 401 from
  `corehttp.RequireStore`. What was missing was AUTHORIZATION — nothing asked
  whether the caller was the customer the path named — and naming it wrongly
  sent a reader looking for an absent 401. The eight storefront routes that name
  a customer (six address, two profile) now resolve `corehttp.Identity` from the
  container and refuse unless what it proves equals what the path claims. gobit
  still issues no identity: an installation that binds none loses those eight
  routes rather than serving them, which is the ADR 0007 row for an unconfigured
  authenticator. Two things this does NOT close and neither is a leftover: the
  cart's `customer_id` (ADR 0051's open defect, still open, still measurable)
  and the second copy of the same boundary in `internal/modules/b2b/api`, which
  ADR 0043 names in its own consequences.
