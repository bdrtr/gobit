...
| A6 | ~~**`allow_backorder`: give it a reader or stop publishing it**~~ **ANSWERED 2026-09-07: ADR 0048 — all four get a reader:** the stock pair in the checkout saga, the product pair in the campaign engine. **Two corrections the check pass forced into the record, both of the class this week is about:** the draft credited `TestEveryColumnIsWrittenBySomething` with proving the boolean columns are written, and that gate explicitly DISCLAIMS it — `auditColumns` skips any column the database supplies, every one of these is `NOT NULL DEFAULT`, so none is inside that audit and it would stay green if one stopped being written. The fact is true and is now stated directly, checked column by column against the query files. The second: the cost of hoisting the variant-to-product hop was stated as ONE additional batch Graph unconditionally, when the tax leg pays it on only one of three paths — the record now gives the cost per tax path | pre-order; today a published flag does nothing, and the flags measurement names it as the live proof of ADR 0009's second error class | Commerce models |
...
| A8 | ~~**Catalog cacheability**~~ **ANSWERED 2026-09-07: ADR 0044 — the sales channel moves into the PATH,** and the publishable key becomes a gate rather than an input to the answer. That is what makes a shared cache possible at all: today the channel arrives out of band, so one storefront's catalog and another's are the same URL with different answers, which is the thing the GraphQL handler's own godoc says a shared cache cannot store | edge caching. The repo already argued the default answer in the GraphQL handler | Storefront speed |
