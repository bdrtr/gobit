...
| A9 | ~~**Customer identity: does ADR 0008 stand or is it superseded?**~~ **ANSWERED 2026-09-07: ADR 0043 — it STANDS.** gobit requires an identity at the address book and still does not issue one: an Identity contract is published, and the address book's identity claim becomes mandatory rather than a path parameter the client chooses. The trust boundary ADR 0008 drew is unchanged; what changes is that the embedder now has a NAMED seam to satisfy it | customer login, passkeys, a customer-facing assistant, A/B assignment, and it is why the address book is unauthenticated | Storefront speed |
