...
| B17 | **KVKK erasure, export and retention** — ~~A2 and A4 come first~~ **both answered 2026-09-06, so this row now has a SHAPE rather than a blocker of its own.** ADR 0029 says what gobit owes: a contract whose outcomes are DELETED, ANONYMIZED and RETAINED, hooks so a module can report an erasure, and a declaration of what each module holds. ADR 0032 supplies the first module's answer — an invoice returns RETAINED and the schema enforces it. What remains is B8: the customer module publishes no event, so nothing can learn an erasure happened, and Principle 2.2 forbids the cascade | a legal requirement; B8 is now the only thing in front of it | Turkey-specific |
